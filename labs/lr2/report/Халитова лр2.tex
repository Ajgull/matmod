...
\begin{document}
	...
	\begin{equation}
		\begin{cases}
			\dfrac{dx}{dt} = a(y-x) = f, \\[12pt]
			\dfrac{dy}{dt} = bx + y - xz = g, \\[12pt]
			\dfrac{dz}{dt} = -\dfrac{8}{3}z + xy = h.
		\end{cases}
	...
	\begin{equation}
		\vec{q} = (\dot{x}, \dot{y}, \dot{z})^T, \quad \operatorname{div} \vec{q} < 0.
		\label{sys:dissipative_conditions}
	\end{equation}
	
	Подставляя ~\eqref{sys:main_equations} в ~\eqref{sys:dissipative_conditions}, получим условие, при котором система будет диссипативной:
	
	\begin{equation}
		\operatorname{div} \vec{q} = -a - \frac{5}{3}, \quad \Rightarrow \quad a > -\frac{5}{3}.
		\label{sys:dissipative_system}
		\nonumber
	\end{equation}
	
	\subsection{Стационарные точки системы}
	
	Стационарные точки системы находятся из решения системы уравнений:
	...
	\subsubsection{Точка $O_1$}
	
	Рассмотрим точку $O_1$, для которой матрица ~\eqref{sys:right_part_matrix} будет иметь вид:
	...
	\begin{equation}
		H = \begin{pmatrix}
			\frac{5}{3} + a & 1 & 0 \\[1ex]
			-\frac{8}{3}a(b+1) & \frac{5}{3}a - \frac{8}{3} + ab & \frac{5}{3} + a \\[1ex]
			0 & 0 & -\frac{8}{3}a(b+1)
		\end{pmatrix}
		\label{eq:gurvits_matrix}
		\nonumber
	\end{equation}
	...
	\begin{equation}
		H_3 = \begin{pmatrix}
			\frac{5}{3}+a & 1 & 0 \\[1ex]
			\frac{16}{3}a(b+1) & \frac{8}{3}(a+b) & \frac{5}{3}+a \\[1ex]
			0 & 0 & \frac{16}{3}a(b+1)
		\end{pmatrix}
		\label{eq:gurvits_matrix}
		\nonumber
	...
	\subsubsection{Точка $O_3$}
	
	Рассмотрим точку $O_3 = \left( - \sqrt{\frac{8}{3}(b+1)}; - \sqrt{\frac{8}{3}(b+1)}; b+1 \right)$. Характеристическое уравнение для этой точки будет иметь вид:
	
	\begin{equation}
		\lambda^3+\lambda^2\left(\frac{5}{3}+a\right)+\lambda\frac{8}{3}\left(a+b\right)+\frac{16}{3}a(b+1)=0.\nonumber
	\end{equation}
	
	Так как характеристические уравнения для точек $O_2$ и $O_3$ совпадают, то области устойчивости этих точек тоже будут совпадать.
