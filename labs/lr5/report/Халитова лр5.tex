...
\begin{document}
	...
	Эксперименты проводились на классическом автомате "Жизнь" с набором правил B2/S012 (рождение при двух соседях, выживание 0, 1 или 2 соседях).
	
	На рисунке \ref{fig:liveinit} показано начальное положение системы. Красным цветом отмечены активные клетки, черным – пассивные.
	...
	Состояние системы после 3 тактов показано на рисунке \ref{fig:live3}.
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.8\linewidth]{../screenshots/live_3}
		\caption{Система после 3 итераций}
		\label{fig:live3}
	\end{figure}
	...
	В ходе реализации клеточного автомата «Жизнь» были подобраны начальные распределения, при которых образуются стационарные и циклические структуры.
	
	В клеточном автомате «Нейронная сеть» возбуждённые клетки передают своё состояние соседним. Наблюдаются круговые волны, которые создает периодический источник возбуждения. Параллельные волны при столкновении «гасят» друг друга, при столкновении диагональных волн, они продолжают своё движение, как единое целое. 
	
%	При реализации клеточного автомата «Организмы – питательная среда» были выявлены следующие особенности:
%		особи клеточного автомата менее жизнеспособны, чем особи модификации этого же алгоритма;
%		в модифицированном клеточном автомате особи распределяются по пространству равномернее;
%		модифицированному алгоритму свойственно явление самоорганизации.
